 ...
 * - LastEditTime: 2024-07-06 21:53:38
 * - Description: 定时器驱动程序
 * - FilePath: \6-2 定时器定时中断与定时器外部时钟\Timer\Timer.c
 **********************************************************************************************************************************/ 
#include "Timer.h"
...
void Timer_Init(void)
{
	...
	{
		.TIM_Prescaler = 7200 - 1, /* 时基单元的预分频器的值，对输入的时钟信号CK_PSC进行7200分频，定时器时钟CK_CNT为即每秒 72M / 7200 = 10000 个周期信号方波 */
		.TIM_CounterMode = TIM_CounterMode_Up, /* 计数方式，向上\下\中央对齐 */
		.TIM_Period = 20000 - 1, /* ARR自动重装器的值，即计数器目标数值为10000，也就是说使用1M的频率，计数10000个，也就是定时为1s*/
